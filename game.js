...
// ABLY
const ably = new Ably.Realtime("kl-cqA.ucrYhg:ECzv8AHG9z3XIFia6nr8ZF9z4b-wfJc_iFlftdxZ7a0");
...
// ===================
// RESET BUTTON (DOUBLE CLICK FUNCTION)
// ===================
resetBtn.addEventListener("click",()=>{
  const now=Date.now();
  if(now-lastResetTime<1000){
    // double click: restart everything
    location.reload();
  }else{
    // single click: clear board/guesses only
    for(let r=0;r<6;r++){
      const row=board.children[r];
      for(let c=0;c<5;c++){row.children[c].textContent=""; row.children[c].className="tile";}
    }
    currentRow=0; currentGuess=""; keyStates={};
    updateKeyboardColors();
    gameOver=false;
    lastResetTime=now;
    publishState();
