...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let confettiTaperMs = 3000;
  const DEFAULT_CONFETTI_DURATION = 30000;
  ...
  function startConfetti(durationMs = DEFAULT_CONFETTI_DURATION, taperMs = 3000){
    ...
    function frame(now){
      ...
      if (timeLeft <= 0) spawnMultiplier = 0;
      else if (timeLeft <= confettiTaperMs) spawnMultiplier = Math.max(0, timeLeft / confettiTaperMs);
      const baseSpawn = 6;
      const toSpawn = Math.round(baseSpawn * spawnMultiplier);
      for (let i=0;i<toSpawn;i++){
        confettiParticles.push({
          x: Math.random() * confettiCanvas.width,
          y: -10 - Math.random()*confettiCanvas.height*0.15,
  ...
  function taperConfettiStop(taperMs = 2000){
    if (!confettiRunning) return;
    confettiEndTime = performance.now() + taperMs;
    confettiTaperMs = taperMs;
  }
  ...
  if (ablyKey) {
    ...
    } catch (err) { console.warn('Ably init failed', err); channel=null; }
  }
  ...
  function evaluateGuess(guess,answer){
    ...
    for(let i=0;i<5;i++){if(res[i]!=='correct'){const idx=arr.indexOf(guess[i]);if(idx!==-1){res[i]='present';arr[idx]=null;}}}
    return res;
  }
  ...
  function submitGuess(){
    ...
    if(currentRow>=6){gameOver=true;publishState(false);showEndOverlay(false);return;}
    publishState(false);
  }
